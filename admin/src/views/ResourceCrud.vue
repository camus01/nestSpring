...
@Component({}) 
export default class ResourceList extends Vue {//data -->  服务端接口
  @Prop(String) resource!:string //强制解析
  ...
  uploadPreview(file,column){
      this.$message.success('请删除现有图片')
    }

  async fetch() {
    const res = await this.$http.get(`${this.resource}`,{
      params: {
        query: this.query//axios : obj --> string
      }
    })
    this.data = res.data
